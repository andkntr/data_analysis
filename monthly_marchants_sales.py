...
#データ読み込み
df = pd.read_excel('/Users/Ken/Downloads/merchants_list.xlsx', engine='openpyxl')

#広告主IDのリスト化
merchant_id = list(df.iloc[:,1])


merchants_name2 = merchant_id
merchants_sales = []
...
try:
    for i in merchants_name2:
        ...
        weeklysales = driver.find_element_by_xpath('//*[@id="page-wrapper"]/div[2]/div/div[4]/div[3]/div/div[1]/h1')
...
df2 = pd.DataFrame(merchants_name2, columns=['Merchant ID'])
df3 = pd.DataFrame(merchants_sales, columns=['Monthly Sales'])


df2[1]=df3
df4 = pd.merge(df,df2, on='Merchant ID', how='outer')
df4.to_excel('/Users/Ken/Downloads/merchants_sales_Jan.xlsx')

"""
now = datetime.date.today()
df.to_excel('/Users/Ken/Downloads/merchants_monthly_sales'+now.strftime('%m%d%Y')+'.xlsx')
